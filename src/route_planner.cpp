...
// - Return the pointer.
bool find(RouteModel::Node*a, RouteModel::Node*b){
    return ((*a).h_value+(*a).g_value)>((*b).h_value+(*b).g_value);
}

RouteModel::Node *RoutePlanner::NextNode() {
    sort(open_list.begin(), open_list.end(),find);
...
// Tips:
// - Use the AddNeighbors method to add all of the neighbors of the current node to the open_list.444
